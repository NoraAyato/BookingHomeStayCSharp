...
namespace DoAnCs.Areas.Host.Controllers
{
    ...
    public class RoomController : Controller
    {
        ...
        public RoomController(IPhongRepository phongRepo, IServiceRepository dichVuRepo, ApplicationDbContext context)
        {
        ...
        public async Task<IActionResult> Index(string homestayId)
        {
            ...
            ViewBag.TienNghiList = await _context.TienNghis
        ...
        public async Task<IActionResult> Create(Phong phong, List<IFormFile> HinhAnhPhongFiles, List<ChiTietPhong> ChiTietPhongs, int MainImageIndex = 0, List<string> ImageDescriptions = null)
        {
            try
            {
                ...
                phong.Ma_Phong = Guid.NewGuid().ToString("N").Substring(0, 20);

                if (await _context.Phongs.AnyAsync(p => p.Ma_Phong == phong.Ma_Phong))
                {
